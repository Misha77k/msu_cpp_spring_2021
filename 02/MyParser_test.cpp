...
void OneStrSymbTest()
{
    ...
    obj4.Parse(text);
    std::stringstream ss;
    for(uint64_t i = 0; i < ArrOfDig.size(); i++) {
        assert (typeid(ArrOfDig[i]) == typeid(uint64_t));
        ss << ArrOfDig[i];
        assert (text.find_first_of(ss.str()) != std::string::npos);
    }
    for(uint64_t i = 0; i < ArrOfStr.size(); i++) {
        assert (typeid(ArrOfStr[i]) == typeid(std::string));
        assert (text.find_first_of(ArrOfStr[i]) != std::string::npos);
    }    
...
void BoundTest()
{
    ...
    ArrOfDig.clear();
    TokenParser obj8;
    std::string text("18446744073709551615  18446744073709551616");
    ...
    assert ((ArrOfDig.size() == 1) && (ArrOfStr.size() == 1));
}

void InsignZerosTest()
{
    ArrOfStr.clear();
    ArrOfDig.clear();
    TokenParser obj9;
    std::string text(" 000123  ");
    obj9.SetDigitTokenCallback(DigitFunc);
    obj9.SetStringTokenCallback(StringFunc);
    obj9.Parse(text);
    std::stringstream ss;
    for(uint64_t i = 0; i < ArrOfDig.size(); i++) {
        assert (typeid(ArrOfDig[i]) == typeid(uint64_t));
...
int main()
{
    ...
    std::cout << "InsignZerosTest" << std::endl;
    InsignZerosTest();
    std::cout << "Success" << std::endl;
